...
-- Trigger to add default columns to new user

CREATE OR REPLACE FUNCTION add_default_columns()
RETURNS TRIGGER AS $$
BEGIN
  -- Insert predefined columns for the new user
  INSERT INTO columns (user_id, name, position, color) 
  VALUES 
    (NEW.id, 'Interested', '0|azzzzz:', '#0693E3'),
    (NEW.id, 'Pending', '0|bzzzzz:', '#9900ef'),
    (NEW.id, 'In Progress', '0|czzzzz:', '#008B02'),
    (NEW.id, 'Offer', '0|dzzzzz:', '#37D67A'),
    (NEW.id, 'Declined', '0|ezzzzz:', '#f47373'),
    (NEW.id, 'Ghosted', '0|fzzzzz:', '#abb8c3'),
    (NEW.id, 'Follow Up', '0|gzzzzz:', '#ff6900');
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER columns_after_user_insert
AFTER INSERT ON users
FOR EACH ROW
EXECUTE FUNCTION add_default_columns();
